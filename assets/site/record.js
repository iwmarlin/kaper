...
function renderPerson(person, data, indexes) {
  ...
  return {
    ...
    label: "Person",
    badges: `${typeBadge(person.primaryRole)}${(person.roles || []).map(typeBadge).join("")}`,
    ...
    main: [
      ...
      section("Works", entityList(works, "work", (item) => [item.year, item.workType].filter(Boolean).join(" · "))),
      section("Contributions", contributionList(contributions, indexes)),
    ...
    aside: `<div class="scope-note">This public authority record connects historical name forms and documented contributions without replacing the wording printed in sources.</div>`,
  };
}
